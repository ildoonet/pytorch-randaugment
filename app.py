...
#         )
        ...
        term_history_str = "\n".join(self.term_history[-6:])
        completion = openai.Completion.create(
            engine="text-davinci-003", 
            prompt=f"""너는 검색 쿼리를 제안하는 어시스턴트이다. 하나의 검색 쿼리를 제안해라. 그냥 단어만 쓰고 불필요한 말은 하지 말라. 검색이 필요없다고 생각하면 그냥 빈 문자열을 써라. 사용자의 메시지와 같은 언어를 사용하면 좋다. 각 단어는 간단하지만, 마지막 메시지에 대한 응답에 유용해야 한다. 히스토리를 보고 쿼리를 구체화할 필요도 있다
----------
강남역 맛집 추천해줘
- 강남역 맛집
answer: ...
...
-""", temperature=0.7,
        ...
        keywords = completion['choices'][0]['text']
        ...
        self.term_history.append(msg)
        self.term_history.append(keywords)
        ...
        references.extend(google_serp(keywords)['response'])
        ...
        self.term_history.append('answer: ' + response)
